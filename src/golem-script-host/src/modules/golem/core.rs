    ...
    fn try_from_js(value: &JsValue, context: &mut Context) -> JsResult<Self> {
        match value {
            JsValue::Object(object) => {
                match object.get(js_string!("type"), context)?.to_string(context)?.to_std_string_escaped().as_str() {
                    "path" => {
                        let path = object.get(js_string!("path"), context)?.to_string(context)?;
                        Ok(CoreType::Path { path })
                    }
                    _ => Err(JsError::from_opaque(js_string!("Invalid core type.").into()))
                }
            }
            _ => Err(JsError::from_opaque(js_string!("Invalid core type.").into()))
        }
...
fn run_(
    ...
    if options.autoloop.unwrap_or(true) {
        run_core_loop(&mut *app, &mut core, true);
    }
}
...
pub fn create_module(context: &mut Context) -> JsResult<(JsString, Module)> {
    Ok((js_string!("core"),
        [
            (js_string!("run"), run_.into_js_function_copied(context)),
