...
fn logger(msg: LogMessage, console_state: &Console, context: &Context) {
    let indent = 2 * console_state.groups.len();
    let frame = if tracing::enabled!(tracing::Level::TRACE) {
        context
            .stack_trace()
            .last()
            .map(|f| f.code_block().name().to_std_string_escaped())
    } else {
        None
    };

    match (frame, msg) {
        (Some(frame), LogMessage::Error(msg)) => {
            tracing::error!(frame, "{msg:>indent$}");
        }
        (Some(frame), LogMessage::Warn(msg)) => {
            tracing::warn!(frame, "{msg:>indent$}");
        }
        (Some(frame), LogMessage::Log(msg) | LogMessage::Info(msg)) => {
            tracing::info!(frame, "{msg:>indent$}");
        }
        (_, LogMessage::Error(msg)) => {
            tracing::error!("{msg:>indent$}");
        }
        (_, LogMessage::Warn(msg)) => {
            tracing::warn!("{msg:>indent$}");
        }
        (_, LogMessage::Log(msg) | LogMessage::Info(msg)) => {
            tracing::info!("{msg:>indent$}");
        }
    }
}
...
impl Console {
    ...
    fn debug(
        ...
        logger(LogMessage::Log(formatter(args, context)?), console, context);
        Ok(JsValue::undefined())
    }
    ...
    fn trace(
        ...
        let stack_trace_dump = context
            ...
            .collect::<Vec<_>>()
            .join("\n");
        logger(LogMessage::Log(stack_trace_dump), console, context);
    ...
    fn count(
        ...
        *c += 1;
        ...
        logger(LogMessage::Info(format!("{msg} {c}")), console, context);
        Ok(JsValue::undefined())
    }
    ...
    fn time_log(
        ...
        console.timer_map.get(&label).map_or_else(
            ...
            |t| {
                ...
                logger(LogMessage::Log(concat), console, context);
            },
        );
